...
static int interpesc(TISRC_DATASTREAM_T ds, int c, char* out) {
    switch (c) {
        case DS_END:
            return -1;
        case 'a':
...
void cfg_read(struct cfg* cfg, TISRC_DATASTREAM_T ds, bool overwrite) {
    struct cfg_sect* sectptr = NULL;
    {
        for (int i = 0; i < cfg->sectcount; ++i) {
            struct cfg_sect* ptr = &cfg->sectdata[i];
...
void cfg_open(TISRC_DATASTREAM_T ds, struct cfg* cfg) {
...
void cfg_merge(struct cfg* cfg, TISRC_DATASTREAM_T ds, bool overwrite) {
    #ifndef TISRC_REUSABLE
    #if !defined(TISRC_REUSABLE) && DEBUG(1)

...
#ifndef TISRC_DEFAULTGAME
    #define TISRC_DEFAULTGAME default
#endif
#ifndef TISRC_DEFAULTLOGO
    #define TISRC_DEFAULTLOGO internal:engine/icon
#endif
...
int bootstrap(void) {
...

    {
        ...
        if (engine.opt.game) {
            ...
            plog(LL_WARN, "No default game specified, falling back to '%s'", STR(TISRC_DEFAULTGAME));
            if (!setGame(STR(TISRC_DEFAULTGAME), false, &err)) {
                plog(LL_CRIT | LF_MSGBOX, "%s", cb_peek(&err));
                cb_dump(&err);
                return 1;
            }

...
#if (PLATFLAGS & PLATFLAG_WINDOWSLIKE) && !defined(TISRC_THREADING_USEWINPTHREAD) && !defined(TISRC_THREADING_USESTDTHREAD)
static DWORD WINAPI threadwrapper(LPVOID t) {
    ((thread_t*)t)->ret = ((thread_t*)t)->func(&((thread_t*)t)->data);
    ExitThread(0);
    return 0;
}
#elif defined(TISRC_THREADING_USESTDTHREAD)
...
static void* threadwrapper(void* t) {
    #if !defined(TISRC_THREADING_NONAMES) && !defined(TISRC_THREADING_USESTDTHREAD)
        if (((thread_t*)t)->name) {
...
bool createThread(thread_t* t, const char* n, threadfunc_t f, void* a) {
    ...
    bool fail;
    #ifndef TISRC_THREADING_USESTDTHREAD
        #if (PLATFLAGS & PLATFLAG_WINDOWSLIKE) && !defined(TISRC_THREADING_USEWINPTHREAD)

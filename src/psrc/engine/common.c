...
void setupBaseDirs(void) {
    if (engine.opt.maindir) {
        dirs[DIR_MAIN] = strpath(engine.opt.maindir);
    } else {
    ...
    if (engine.opt.gamesdir) {
        dirs[DIR_GAMES] = strpath(engine.opt.gamesdir);
    } else {
        dirs[DIR_GAMES] = mkpath(dirs[DIR_MAIN], "games", NULL);
    }
    if (engine.opt.modsdir) {
        dirs[DIR_MODS] = strpath(engine.opt.modsdir);
    } else {
